...
/// Step 8:
///         modify pose:
///             ros2 service call object_manager/modify_pose spawn_object_interfaces/srv/ModifyPose "{frame_name: Ref_Frame_1, rel_pose:{position: {x: -0.035, y: -0.02166, z: 0.00235}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0}}}"
///
/// </summary>
    ...
    void Start()
    {
        ...
        {                        
            if (ros2Node == null)
            {
                ...
                try
                {
                    ModifyPoseService = ros2Node.CreateService<modifyPoseReq, modifyPoseResp>("/object_manager_unity/modify_pose", ModifyPose);
                }
                catch(Exception ex)
                {
