...
using destroyObjReq = assembly_manager_interfaces_unity.srv.DestroyObjectUnity_Request;
using destroyObjResp = assembly_manager_interfaces_unity.srv.DestroyObjectUnity_Response;
...
namespace ROS2
{
    public class Destroy_Object : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (ros2Unity.Ok())
            {
                if (ros2Node == null)
                {
                    ...
                    try
                    {
                        DeleteObjectService = ros2Node.CreateService<destroyObjReq, destroyObjResp>("/assembly_manager_unity/destroy_object", deleteObject);
                    }
                    catch (Exception ex)
                    {
        ...
        public assembly_manager_interfaces_unity.srv.DestroyObjectUnity_Response deleteObject(assembly_manager_interfaces_unity.srv.DestroyObjectUnity_Request msg)
        {
            Debug.Log("About to delete Object with name:" + msg.Obj_name);

            assembly_manager_interfaces_unity.srv.DestroyObjectUnity_Response response = new assembly_manager_interfaces_unity.srv.DestroyObjectUnity_Response();
